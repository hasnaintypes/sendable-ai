...
export const useCreateTodo = () =>
  useMutation(api.todos.mutations.create).withOptimisticUpdate(
    (localStore, args) => {
...
export const useToggleCompleted = () =>
  useMutation(api.todos.mutations.toggle).withOptimisticUpdate(
    (localStore, args) => {
      const todos = localStore.getQuery(api.todos.queries.get);
      if (!todos) {
        return;
...
export const useRemoveTodo = () =>
  useMutation(api.todos.mutations.remove).withOptimisticUpdate(
    (localStore, args) => {
      const todos = localStore.getQuery(api.todos.queries.get);
      if (!todos) {
        return;
