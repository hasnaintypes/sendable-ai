...
const PasswordInput = React.forwardRef<HTMLInputElement, PasswordInputProps>(
  ({ className, showStrength = false, showStrengthOnSubmit = false, value, onChange, ...props }, ref) => {
    ...
    const showIndicator = showStrength && (showStrengthOnSubmit ? attempted : password.length > 0)

    React.useEffect(() => {
      if (!showStrengthOnSubmit) return
      const form = (ref as React.RefObject<HTMLInputElement>)?.current?.closest("form")
      if (!form) return
      ...
      <div>
        ...
        {showIndicator && (
          <>
            ...
            >
              <div
                className={cn("h-full transition-all duration-500 ease-out", getStrengthColor(score))}
              ...
              {strength.map((req, i) => (
                <li key={i} className="flex items-center gap-2">
                  ...
                  <span
                    className={cn(
                      "text-xs",
                      req.met ? "text-emerald-600 dark:text-emerald-400" : "text-muted-foreground/80"
                    )}
                  >
                    {req.text}
                  </span>
